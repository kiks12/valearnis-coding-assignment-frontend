...
const MultiAnswerChoices: React.FC<props> = ({ choices, questionIndex, onChange }) => {
	...
	const onCheckboxChange = (index: number, checked: boolean) => {
		setChoices((prev) =>
			prev.map((choice) => {
				...
				if (choice.index !== index && choice.is_answer) return choice;
				return {
					...choice,
					is_answer: false,
				};
			...
			{choices.length !== 0 &&
				choices.map((choice, idx) => {
					return (
						...
						>
							<input onChange={(e) => onCheckboxChange(choice.index, e.target.checked)} type="checkbox" name={`choice${questionIndex}`} />
							<input
								value={choice.text}
...
export default MultiAnswerChoices;
